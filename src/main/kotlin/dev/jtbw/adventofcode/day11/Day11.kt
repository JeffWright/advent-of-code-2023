...
import dev.jtbw.adventofcode.util.twodeespace._forEachIndexed
...
object Day11 : AoCDay<Grid<Space>> {
  ...
  override fun part2() {
    ...
    runTiming("solving part 2") {
      solve("Day11.txt", expansionFactor = 1_000_000)
  ...
  private fun solve(filename: String, expansionFactor: Long) : Long {
    ...
    grid._forEachIndexed { pos, space ->
      if(space == Galaxy) {
        galaxies += pos
   ...
   */
  private fun distance(from: Offset, to: Offset, emptyColumns: Set<Int>, emptyRows: Set<Int>, expansionFactor: Long): Long {
    ...
    val extraDistanceFromColumns = emptyColumns.count {
      it in xRange
    } * (expansionFactor - 1)
    val extraDistanceFromRows = emptyRows.count {
      it in yRange
    } * (expansionFactor - 1)
